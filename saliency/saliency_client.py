...
#                break
    ...
    def get_dataset(self, held_out_study=None, index_csv="tmp/monsterx.csv"):
        ...
        if not os.path.isfile(self.dataset_descriptor):
            response = requests.get(
                "https://api.saliency.ai/projections/export/",
                auth=(
                    self.login,
                    self.password))
    ...
    def prepare_dataset_held_out(
        ...
        self.labels = [self.studies[i] + "_" + self.labels[i]
...
#         n_remaining = self.n - n_held_out
        ...
        for lab in np.unique(self.remaining_labels):
            ...
            self.seq["val"].append(
                np.array(indices[lab_ntrain:(lab_ntrain + lab_nval)]))
            self.seq["test"].append(
                np.array(indices[(lab_ntrain +
                                  lab_nval):(lab_ntrain +
                                             lab_nval + lab_ntest)]))
        ...
        # Process labels
        self.keys = dict([(b, a) for a, b in enumerate(
            list(set(self.remaining_labels)))])
        self.label_dict = dict(
            [(a, b) for a, b in enumerate(list(set(self.remaining_labels)))])
        self.Y = np.array([self.keys[lab] for lab in self.remaining_labels])

        self.keys_held_out = dict(
            [(b, a) for a, b in enumerate(list(set(self.held_out_labels)))])
        self.Y_held_out = np.array([self.keys_held_out[lab]
                                    for lab in self.held_out_labels])
...
#         X = Xtmp
#         if split == 'held_out':
#             Y = keras.utils.to_categorical(self.Y_held_out[ids],
#                       num_classes = len(np.unique(self.Y_held_out)))
#         else:
#             Y = keras.utils.to_categorical(self.Y[ids],
#                       num_classes = len(np.unique(self.Y)))
#         return X, Y, np.array([1]*len(ids))

    def get_batch_held_out(
            self,
            index,
            split,
            batch_size,
            channels_first,
            augmentations=[]):
        # helper function only to be used with the keras
        # data generator...NOT the pytoch generator
        sq = self.seq[split]
        ...
        if split == 'held_out':
            Y = tensorflow.keras.utils.to_categorical(
                self.Y_held_out[ids_list],
                num_classes=len(
                    np.unique(
                        self.Y_held_out)))
        else:
            Y = tensorflow.keras.utils.to_categorical(
                self.Y[ids_list], num_classes=len(np.unique(self.Y)))

        sample_weights = np.array([self.class_weight_dict[i]
                                   for i in self.Y[ids_list]])
        ...
        if self.framework == 'keras':
            ...
            class SaliencyDataGenerator(tensorflow.keras.utils.Sequence):
                ...
                def __getitem__(self, index):
                    return self.sapi.get_batch_held_out(
                        index,
                        self.split,
                        self.batch_size,
                        augmentations=augmentations,
                        channels_first=self.sapi.channels_first)
    # return self.sapi.get_batch(index, self.split, self.batch_size,
    # augmentations = augmentations)
                ...
                def see_one_batch(self):
                    ...
                    for j in np.arange(len(X)):
                        ...
                        plt.colorbar()
                        plt.title(
                            self.sapi.label_dict[np.where(Y[j] == 1)[0][0]])
                        plt.show()
        ...
        if self.framework == 'pytorch':
            ...
            class SaliencyDataset(Dataset):
                ...
                def __len__(self):
                    ...
                    # (differs from keras which denotes BATCHES/epoch)'
                    return len(self.sapi.seq[split])

                def __getitem__(self, index):
                    # Select sample
                    # ID is the location of the item in sapi.remaining_files or
                    # sapi.held_out_files
                    ID = self.sapi.seq[self.split][index]

                    'Load data and get label'

                    """
                    Note: the target for pytorch should be expressed as
                    an int denoting the index of the true class
                    (This differs from Keras, which expects the target
                    to be expressed as a one-hot vector)
                    """
                    if split == 'held_out':
    ...
    def calculate_class_weights(self, power=1):
        ...
        for i in np.arange(len(labels)):
            if power is None:
                ...
                self.class_weight_dict[labels[i]
                                       ] = 1.0 / (label_percent[i]**power)

        class_weight_list = [self.class_weight_dict[i]
                             for i in np.arange(len(self.class_weight_dict))]
